...
import type { WorkflowIntrospectionOperation } from "./testing";
...
// WorkflowBinding instances are not guaranteed to be stable across RPC calls, so
// introspection sessions are scoped by an internal key provided by Miniflare.
...
export class WorkflowBinding extends WorkerEntrypoint<Env> {
	...
	public async unsafeStartIntrospection(
		operations: WorkflowIntrospectionOperation[]
	): Promise<string> {
		if (workflowIntrospectionSessions.has(this.env.INTROSPECTION_KEY)) {
			throw new Error(
		...
		workflowIntrospectionSessions.set(this.env.INTROSPECTION_KEY, {
			id: sessionId,
			operations,
			instanceIds: [],
		});
	...
	public async unsafeStopIntrospection(sessionId: string): Promise<void> {
		const session = workflowIntrospectionSessions.get(
			this.env.INTROSPECTION_KEY
		);
		if (session?.id === sessionId) {
			workflowIntrospectionSessions.delete(this.env.INTROSPECTION_KEY);
		}
	}
